...
# Use a temporary directory for NLTK downloads
nltk_data_path = tempfile.gettempdir() + "/nltk_data"
...
connection = pymysql.connect(host='localhost',user='root',password='sairam123',db='cv')
...
def run():
    ...
    link = '[©Developed by Sai Ram Naraboina](https://www.linkedin.com/in/sairam-naraboina/)'
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'sairam' and ad_password == 'sairam123':
                st.success("Welcome Sai Ram Naraboina!")
                # Display Data
                # Function to decode byte-like objects and format lists as comma-separated strings
                def decode_bytes(value):
                    if isinstance(value, bytes):  # Convert bytes to string
                        value = value.decode('utf-8')  
                    if isinstance(value, str):  
                        value = value.strip()  # Remove unnecessary spaces
                        if value.startswith("[") and value.endswith("]"):  
                            value = value[1:-1]  # Remove square brackets
                        value = value.replace("'", "")  # Remove single quotes around words
                ...
                # Apply decoding function to clean up the data
                ...
                st.dataframe(df)
                st.markdown(get_table_download_link(df.astype(str), 'User_Data.csv', 'Download Report'), unsafe_allow_html=True)
                ...
                plot_data = pd.read_sql(query, connection, dtype=str)  # Ensure all data is treated as strings




                # Decode any byte-like objects into normal strings
                # Ensure proper decoding of stored values
                for col in ['Predicted_Field', 'User_level', 'Actual_Skills', 'Recommended_Skills', 'Recommended_Course']:
                    if col in plot_data.columns:
                        plot_data[col] = plot_data[col].apply(lambda x: x.decode('utf-8') if isinstance(x, bytes) else str(x))
